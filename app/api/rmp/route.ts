...
async function fetchRatingFromRMP(lastName: string): Promise<RMPRating | null> {
  try {
...
async function getRating(name: string): Promise<RMPRating | null> {
  ...
  const cacheKey = `rmp:${lastName.toLowerCase()}`;
  ...
  const rating = await fetchRatingFromRMP(lastName);
